...
def print_loss_acc(fitHistory, i, type):
	#  ######### Print Loss Map ##########
	plt.figure()
	...
	plt.savefig('./curve4/' + str(type) + 'loss%d.png' % i)
...
def print_acc(fitHistory, i, type):
	#  ######### Print Loss Map ##########
	plt.figure()
	plt.plot(fitHistory.history['acc'])
	plt.plot(fitHistory.history['val_acc'])
	...
	plt.savefig('./curve4/' + str(type) + 'acc%d.png' % i)
...
def zyh_CNN(trainX, trainY,valX=None, valY=None, compiletimes = 0, forkinas=False,transferlayer=1,compilemodels=None,earlystop=None,nb_epoch=500,batch_size = 2048 ):
	"""
	:argument
	:return:
	"""
	...
	print_loss(fitHistory, compiletimes, 'loss_')
	print_acc(fitHistory, compiletimes, 'acc_')
	train_loss1, train_acc1, val_loss2, val_acc2 = print_loss_acc(fitHistory, compiletimes, 'loss_acc_')
