...
def evaluate_model(modelname, testX, testY, i, type):
	...
	fileX = open('./curve4/pre_score%d.pickle' % i, 'wb')
	...
	plt.savefig('./curve4/Precision-Recall%d.png' % i)
	...
	plt.savefig('./curve4/ROC %d.png' % i)
...
def using_one_of_ten_fold_crossing(one_of_ten_fold_file_path,x_th_type):
	...
	for nclass_times in range(len(train_neg)):
		...
		if nclass_times == 0:
			pass
			models,accuracys = zyh_CNN(trainX1, trainY1, valX1, valY1, compiletimes=nclass_times,transferlayer=1,forkinas=1)
		else:
			pass
			models,accuracys = zyh_CNN(trainX1, trainY1, valX1, valY1, compiletimes=nclass_times, transferlayer=1,forkinas=1,compilemodels=models)
			pre_score, pre, rec, SN, SP, f1, mcc, roc_auc = evaluate_model(models, valX1,valY1, nclass_times, "6")	
			...
			print_final_acc_loss(accuracys_final, nclass_times, x_th_type)	
...
def print_final_acc_loss(accuracys_final, i, type):
	#  ######### Print Loss Map ##########
	plt.figure()
	...
	plt.savefig('./curve4/' + str(type) + 'loss_acc%d.png' % i)
...
def using_ten_fold_crossing(ten_fold_file_path):
	...
	compiletimes = 0
	for dir in dirs:
	    models = using_one_of_ten_fold_crossing(ten_fold_file_path + str(dir) + "/" , str(dir)+ "_")
