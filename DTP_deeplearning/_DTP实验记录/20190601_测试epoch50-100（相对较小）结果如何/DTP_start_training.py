...
def using_one_of_ten_fold_crossing(one_of_ten_fold_file_path,x_fold_type,fildername):
	...
	for nclass_times in range(len(train_neg)):
		...
		pre_score, pre, rec, SN, SP, f1, mcc, roc_auc = evaluate_model(models, valX1,valY1, nclass_times, "5",fildername)	
...
def using_ten_fold_crossing(ten_fold_file_path):
	...
	compiletimes = 0
	for dir in dirs:
	    models = using_one_of_ten_fold_crossing(ten_fold_file_path + str(dir) + "/" , str(dir)+ "_","result_5")
