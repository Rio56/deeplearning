...
def get_data():
	...
	file_path = "C:\\Users\\_Rio56\\Desktop\\drug_target_gpu_test\\zyh_data\\"
...
def prepare_data(data_with_flag):
	...
	#shuffer
	#train_pos = train_pos.sample(train_pos.shape[0])
	#train_neg = train_neg.sample(train_neg.shape[0])
	
	#pos_test_number = int(train_pos.shape[0] * 0.9)  # a = 90% positive.
	#neg_test_number = train_neg.shape[0] - int(train_pos.shape[0] * 0.1)  # b = negtive - 10% positive.
	#train_pos_data = train_pos[0:pos_test_number]  # choose the former 90% positive
	#train_neg_data = train_neg[0:neg_test_number]  # choose the former 100% - (0.1 * pos) negtive
	
	# val dataset
	#val_pos_data = train_pos[(pos_test_number + 1):];
	#val_neg_data = train_neg[(pos_test_number+ 1):];
...
def save_Bootstrapping_data(data,tenfold_list,tenfload,times,srate = 0.8,tag = "tag"):
	path = 'C:\\Users\\_Rio56\\Desktop\\drug_target_gpu_test\\zyh_DrugTargetPrediction\\DrugTargetPrediction_data\\ten_fold_data\\'
	path = path + str(tenfload) + '\\'

	newdata =pd.DataFrame(columns = data.columns.values.tolist())
	counter = 1
	nclass_times =0
	#slength = int(slength * srate)
	...
	"""slength = int(newdata.shape[0]/times)
	print(slength)
	for nclass_times in range(times):
		newdata = newdata[(slength * nclass_times):(slength * nclass_times + slength)]
		print(newdata)
		name = path + 'tenfold_' + str(tenfload) + '_' + tag +str(nclass_times) + '.npy'
		np.save(name,newdata)			"""
