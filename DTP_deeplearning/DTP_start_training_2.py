...
from methods.DTP_set_Net import zyh_CNN,zyh_CNN_one_hot_flying
...
def using_one_of_ten_fold_crossing(one_of_ten_fold_file_path, x_fold_type, fildername):
    ...
    models, accuracys = zyh_CNN(trainX1, trainY1, valX1, valY1, compiletimes=0, transferlayer=1, forkinas=1,
    ...
    while compile_flag < 150:
        for nclass_times in range(len(train_neg)):
            models, accuracys = run_model(nclass_times, compile_flag, one_of_ten_fold_file_path, fildername, models,
                                          train_neg, train_pos, val_neg, val_pos)

        pre_score, pre, rec, SN, SP, f1, mcc, roc_auc = evaluate_model(models, valX1, valY1, nclass_times, x_fold_type,
                                                                       fildername)
...
def run_model(nclass_times, compile_flag, one_of_ten_fold_file_path, fildername, models, train_neg, train_pos, val_neg,
              val_pos):
    path = one_of_ten_fold_file_path
    ...
    # print(val_all)
    valX1, valY1 = convertRawToXY(val_all.as_matrix(), codingMode=codingMode)
    models, accuracys = zyh_CNN(trainX1, trainY1, valX1, valY1, compiletimes=compile_flag, transferlayer=1, forkinas=1,
                                compilemodels=models, fildername=fildername)
...
def start_training_2():
    Data_prepare = DTP_prepare_data()
    train_x, train_y = Data_prepare.load_data("train","one_hot" ,n_to_p_rate=1, random_rate=0.8, system="windows")
    val_x, val_y = Data_prepare.load_data("val","one_hot" ,n_to_p_rate=1, random_rate=0.8, system="windows")
    #test_x, test_y = Data_prepare.load_data("test", n_to_p_rate=1, random_rate=0.8, system="windows")

    fildername = "result"
    accuracys_final = {'train_loss1': [], 'train_acc1': [], 'val_loss2': [], 'val_acc2': []}

    models, accuracys = zyh_CNN_one_hot_flying(train_x, train_y, val_x, val_y, compiletimes=0, transferlayer=1, forkinas=1,
    fildername = fildername)

    for item in range(0, 50):
        train_x, train_y = Data_prepare.load_data("train", "one_hot",n_to_p_rate=1, random_rate=0.8, system="windows")
        val_x, val_y = Data_prepare.load_data("val", "one_hot",n_to_p_rate=1, random_rate=0.8, system="windows")
        models, accuracys = zyh_CNN_one_hot_flying(train_x, train_y, val_x, val_y, compiletimes=item, transferlayer=1, forkinas=1, compilemodels=models,
                                    fildername=fildername)
        pre_score, pre, rec, SN, SP, f1, mcc, roc_auc = evaluate_model(models, val_x, val_y, item, "__", fildername)
        accuracys_final['train_loss1'].append(accuracys['train_loss1'])
        accuracys_final['train_acc1'].append(accuracys['train_acc1'])
        accuracys_final['val_loss2'].append(accuracys['val_loss2'])
        accuracys_final['val_acc2'].append(accuracys['val_acc2'])
        print_final_acc_loss(accuracys_final, item, "__", fildername)
        print("pre_score, pre, rec, SN, SP, f1, mcc, roc_auc")
        print(pre_score, pre, rec, SN, SP, f1, mcc, roc_auc)
